...
export default function FitnessDietScreen() {
    ...
    const getRandomFallback = () => {
        ...
        const fallbackOptions = [
            ...
            { mealName: "Dal Tadka with Rice", reason: "Balanced meal with protein-rich lentils.", calories: 450, protein: 18 },
            { mealName: "Paneer Bhurji with Roti", reason: "Excellent source of calcium and protein.", calories: 420, protein: 22 },
            { mealName: "Mixed Fruit Chaat", reason: "Light, refreshing and easy to digest.", calories: 120, protein: 2 },
    ...
    const fetchSmartSuggestion = async () => {
        ...
        try {
            ...
            console.log('[Diet] Fetching meal suggestion...', { historyCount: history.length, currentHour });
            ...
            console.error('[Diet] Failed to fetch suggestion:', error.response?.data || error.message);
    ...
    const handleAddMeal = async () => {
        ...
        try {
            ...
            LPHaptics.success();
            fetchSmartSuggestion();
            Alert.alert("Meal Added", `Logged ${newMeal.name} (${newMeal.calories} kcal)`);

        } catch (error: any) {
            LPHaptics.error();
            const errorMessage = error.response?.data?.error || error.response?.data?.message || "Could not analyze food. Please check your connection.";
        ...
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                <View>
                    ...
                    <Text style={styles.headerSubtitle}>Fuel your body for performance</Text>
                </View>
            ...
            <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
                ...
                { }
                <Animated.View entering={FadeInDown.delay(100).duration(500)} style={styles.suggestionCard}>
                    <LinearGradient
                    ...
                    >
                        ...
                        <View style={{ flex: 1 }}>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <Text style={styles.suggestionTitle}>Chef's Recommendation</Text>
                                {loadingSuggestion && <ActivityIndicator size="small" color="#FFF" />}
                            </View>

                            {suggestion ? (
                                <>
                                    <Text style={styles.suggestionMealName}>{suggestion.mealName}</Text>
                                    <Text style={styles.suggestionText}>{suggestion.reason}</Text>
                                </>
                            ) : (
                                <Text style={styles.suggestionText}>Loading personalized tip...</Text>
                            )}
                        </View>
                    </LinearGradient>
                </Animated.View>
                ...
                { }
                <Animated.View entering={FadeInDown.delay(200).duration(500)} style={styles.summaryContainer}>
                ...
                <View style={styles.section}>
                    <View style={styles.sectionHeader}>
                        ...
                        {meals.length === 0 && <Text style={styles.emptyText}>No meals logged yet</Text>}
                    </View>

                    {meals.map((meal, index) => (
                        <Animated.View key={meal.id} entering={FadeInDown.delay(300 + (index * 50)).duration(400)} style={styles.mealItem}>
                            <View style={styles.mealInfo}>
                                <Text style={styles.mealName}>{meal.name}</Text>
                                <Text style={styles.mealDetails}>{meal.calories} kcal • {meal.protein}g protein</Text>
                            </View>
                            <View style={styles.mealRight}>
                                <Text style={styles.mealTime}>{meal.time}</Text>
                                <TouchableOpacity onPress={() => handleDeleteMeal(meal.id)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                    ...
                    <TouchableOpacity style={styles.logMealBtn} onPress={() => setShowAddModal(true)}>
                    ...
                    >
                        <View style={styles.waterHeader}>
                            <View>
                                ...
                                <Text style={styles.waterSubtitle}>Stay hydrated & energized</Text>
                            </View>
                ...
                { }
                <Animated.View entering={FadeInDown.delay(500).duration(500)} style={[styles.section, { marginBottom: 100 }]}>
                    ...
                    <View style={styles.swapCard}>
                        ...
                        <Ionicons name="arrow-forward" size={24} color={LPColors.textGray} />
                        <View style={styles.swapItem}>
                            <Text style={styles.swapLabel}>Try</Text>
                            <Text style={[styles.swapFood, { color: LPColors.primary }]}>Green Tea 🍵</Text>
            ...
            <Modal visible={showAddModal} transparent animationType="slide">
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <Text style={styles.modalSub}>Describe what you ate, and our AI will calculate the nutrients.</Text>
