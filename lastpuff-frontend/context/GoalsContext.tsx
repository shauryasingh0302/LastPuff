...
export const GoalsProvider = ({ children }: { children: ReactNode }) => {
    ...
    const fetchAgenticGoals = async (currentStreak: number): Promise<Goal[]> => {
        try {
            ...
            const storedHealthData = await AsyncStorage.getItem(STORAGE_KEY_HEALTH_DATA);
            const storedFitnessLevel = await AsyncStorage.getItem(STORAGE_KEY_FITNESS_LEVEL);
            const storedBMI = await AsyncStorage.getItem(STORAGE_KEY_BMI);
            const storedCompletedHistory = await AsyncStorage.getItem(STORAGE_KEY_COMPLETED_HISTORY);
            ...
            const completedHistory = storedCompletedHistory ? JSON.parse(storedCompletedHistory) : [];
            ...
            const responseData = response.data as { goals?: Array<{ text: string; icon: string }> };
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const storedFitnessLevel = await AsyncStorage.getItem(STORAGE_KEY_FITNESS_LEVEL);
                if (storedFitnessLevel) setFitnessLevelState(storedFitnessLevel as FitnessLevel);


                const storedWaterDate = await AsyncStorage.getItem(STORAGE_KEY_WATER_DATE);
                ...
                const storedStreakDate = await AsyncStorage.getItem(STORAGE_KEY_STREAK_DATE);
                ...
                const storedGoalsDate = await AsyncStorage.getItem(STORAGE_KEY_GOALS_DATE);
    ...
    const saveCompletedGoalToHistory = async (goalText: string) => {
        try {
            const storedHistory = await AsyncStorage.getItem(STORAGE_KEY_COMPLETED_HISTORY);
            ...
            await AsyncStorage.setItem(STORAGE_KEY_COMPLETED_HISTORY, JSON.stringify(trimmedHistory));
        } catch (error) {
    ...
    const updateWaterIntake = async (count: number) => {
        ...
        await AsyncStorage.setItem(STORAGE_KEY_WATER_DATE, new Date().toDateString());
    };
    ...
    const refreshGoalsWithAI = async () => {
        ...
        setGoals(aiGoals);
        await AsyncStorage.setItem(STORAGE_KEY_GOALS, JSON.stringify(aiGoals));
        await AsyncStorage.setItem(STORAGE_KEY_GOALS_DATE, new Date().toDateString());
    };
